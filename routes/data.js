...
router.post('/plans', authenticate, async (req, res) => {
  try {
    ...
    console.error('[Server] Save plan error:', err);
...
router.get('/plans', authenticate, async (req, res) => {
  try {
    ...
    `;
    const plans = {};
    for (const row of rows) plans[row.id] = { id: row.id, month: row.month, year: row.year, status: row.status, post_count: parseInt(row.post_count) };
...
router.delete('/ai-images/:id', authenticate, async (req, res) => {
  try {
    ...
    console.error('[Server] Delete AI image error:', err);
...
router.put('/ai-images/:id', authenticate, async (req, res) => {
  try {
    ...
    console.error('[Server] Update AI image error:', err);
...
router.get('/slides/plan/:planId', authenticate, async (req, res) => {
  try {
    ...
    console.error('[Server] Load slides error:', err);
